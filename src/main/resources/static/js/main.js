...
function bindNavbarEvents() {
    ...
    if (session && session.isLoggedIn) {
        ...
        if(authActions) {
            ...
            const dropdown = authActions.querySelector('.dropdown');
            if(dropdown && (session.user.role === 'ADMIN' || session.user.email === 'admin@farmconnect.com')) {
    ...
    if (logoutBtn) {
        logoutBtn.addEventListener('click', (e) => {
            e.preventDefault();
            // Cart stays with the user key — just clear session
    ...
    // Initial Cart State update — uses user-scoped cart key
...
(function(d, t) {
    ...
    v.onload = function() {
      window.voiceflow.chat.load({
        verify: { projectID: '669e50f97b19f6aa387df58f' },
